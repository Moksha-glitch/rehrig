  ...
  return (
    <Page>
      <PageHeader
        ...
        description={
          <span>
            <span className="mono tabular-nums">{scopedAccounts.filter((a) => !a.inactive).length}</span> active Â·{' '}
            <span className="mono tabular-nums">{scopedAccounts.length}</span> total
            {drafts.length > 0 && (
              <>
                {' '}
                Â· <span className="mono tabular-nums">{drafts.length}</span> draft
                {drafts.length === 1 ? '' : 's'}
              </>
            )}
            ...
            {!canCreateAccounts && <span className="text-ink-faint"> Â· View only</span>}
          </span>
        }
        actions={
          canCreateAccounts ? (
        ...
        <Panel className="mt-5">
          <div className="flex items-end justify-between border-b border-line px-5 py-4">
            ...
            <span className="text-xs text-ink-muted">
              Saved from onboarding Â· resume anytime
            </span>
          </div>
          <Table
            columns={[
      ...
      {draftToDelete && (
        <ConfirmDialog
          title="Delete draft?"
          description={`Delete â€œ${draftToDelete.name || 'Untitled draft'}â€? This action cannot be undone.`}
          confirmLabel="Delete"
          onConfirm={async () => {
            try {
