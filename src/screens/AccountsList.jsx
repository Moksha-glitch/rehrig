...
import { EditProviderDialog } from './AccountDetail.jsx';
...
export default function AccountsList({ onOnboard }) {
  ...
  const matches = (a) =>
    [a.name, a.uid, a.industry, a.phone, a.ownerName, a.billing?.city, a.billing?.state].some(
      (value) => String(value || '').toLowerCase().includes(q.trim().toLowerCase())
    );

  const activeRows = scopedAccounts.filter((a) => !a.inactive).filter(matches).filter(() => status !== 'Inactive');
  const inactiveRows = scopedAccounts.filter((a) => a.inactive).filter(matches).filter(() => status !== 'Active');
  const drafts = (draftsQuery.data || []).filter((d) => (d.name || '').toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <Page>
      <PageHeader
        ...
        description={
          <span>
            <span className="mono tabular-nums">{scopedAccounts.filter((a) => !a.inactive).length}</span>{' '}
            active &middot;{' '}
            <span className="mono tabular-nums">{scopedAccounts.length}</span> total
            {drafts.length > 0 && (
              <>
                {' '}&middot; <span className="mono tabular-nums">{drafts.length}</span> draft{drafts.length === 1 ? '' : 's'}
              </>
            )}
            {isScoped && state.currentUser?.scopeLabel && (
              <span className="text-ink-faint"> &middot; {state.currentUser.scopeLabel}</span>
            )}
            {!canCreateAccounts && <span className="text-ink-faint"> &middot; View only</span>}
          </span>
        }
        actions={
          canCreateAccounts ? (
      ...
      >
        <Panel>
          <Toolbar>
            <SearchField value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search accounts&hellip;" label="Search accounts" />
            <select aria-label="Filter by status" className="field-input max-w-[160px]" value={status} onChange={(e) => setStatus(e.target.value)}>
              <option>All</option>
              <option>Active</option>
              <option>Inactive</option>
            </select>
            {(q || status !== 'All') && (
              <button type="button" className="link-brand text-xs" onClick={() => { setQ(''); setStatus('All'); }}>
                Clear filters
              </button>
            )}
          </Toolbar>
          ...
          >
            {providerRows.map((a, i) => (
              ...
              >
                ...
                <td className={`truncate px-4 py-3.5 text-ink-muted sm:px-5 ${compact ? 'hidden' : ''}`}>{a.ownerName}</td>
                <td className={`mono px-4 py-3.5 text-ink-muted tabular-nums sm:px-5 ${compact ? 'hidden' : ''}`}>{a.residents}</td>
        ...
        {drafts.length > 0 && (
          <Panel className="mt-5">
            <div className="flex items-end justify-between border-b border-line px-5 py-4">
              ...
              <span className="text-xs text-ink-muted">Saved from onboarding &middot; resume anytime</span>
            </div>
            <Table columns={['#', 'Draft name', 'Industry', { label: 'Last saved', className: compact ? 'hidden' : '' }, { label: 'Owner', className: compact ? 'hidden' : '' }, '', 'Status']}>
              {drafts.map((d, i) => (
                ...
                >
                  ...
                  <td className={`truncate px-4 py-3.5 text-ink-muted sm:px-5 ${compact ? 'hidden' : ''}`}>{d.ownerName || '—'}</td>
                  <td className="px-4 py-3.5 sm:px-5">
                    <div className="flex items-center gap-2">
                      <Button variant="secondary" size="sm" onClick={() => onOnboard?.(d.id)}>Resume</Button>
                      <button type="button" onClick={() => setDraftToDelete(d)} className="rounded-control p-1.5 text-ink-faint interactive hover:bg-danger-soft hover:text-danger" aria-label={`Delete draft ${d.name}`}>
                        <Icon name="x" size={14} />
                      </button>
                    </div>
                  </td>
                  <td className="px-4 py-3.5 sm:px-5"><Badge color="amber">Draft</Badge></td>
                </tr>
              ))}
            </Table>
          </Panel>
        ...
        {editProvider && (
          <EditProviderDialog account={editProvider} onClose={() => setEditProvider(null)} onSaved={() => { setEditProvider(null); accountsQuery.refetch(); }} />
        )}
        {draftToDelete && (
          <ConfirmDialog
            title="Delete draft?"
